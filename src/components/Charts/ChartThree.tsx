...
const ChartThree: React.FC<ChartThreeProps> = ({ title, title1, series1 }) => {
  if (!series1 || series1.length < 2) {
    return null; // Render nothing if series1 is undefined or doesn't have enough elements
  }
    ...
    <div className="col-span-6 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-3">
      <div className="mb-3 justify-between gap-4 sm:flex">
        <div>
          ...
          <h6 className="text-1xl font-semibold text-black dark:text-white" style={{ whiteSpace: 'pre-wrap' }}>{title1}</h6>
        </div>
      </div>
      ...
      <div className="mb-2">
        <div id="chartThree" className="mx-auto flex justify-center">
          <div style={{ width: '200px', height: '200px' }}>
            <ReactApexChart options={options} series={series1} type="donut"/>
          </div>
        </div>
      </div>

...
const processedModuleBarcodes = new Set();
    ...
    try {
        ...
        if (v1statusok || v1statusnotok) {
            ...
            if (result.recordset.length > 0) {
                ...
                if (!processedModuleBarcodes.has(moduleBarcode)) {
                    await pool.close();

                    // Create a new connection pool for the specific query
                    const specificPool = await sql.connect(SqlConfig2);
                    ...
                    const checkResult = await specificPool.request()
                    ...
                    if (checkResult.recordset.length > 0) {
                        ...
                        // Mark the module barcode as processed
                        processedModuleBarcodes.add(module_barcode);
                    } else {
                        console.log(`Module barcode ${moduleBarcode} not found in cell_sorting table.`);
                    }
                } else {
                    console.log(`Module barcode ${moduleBarcode} already processed.`);
                }
            }
        }
    } catch (error) {

...
export async function updateModuleBarcode(req, res) {
    const { scannedBarcode } = req.body;
    if (!scannedBarcode) {
        return res.status(400).json({ error: 'Scanned barcode is required' });
    }
    moduleBarcode = scannedBarcode; // Update moduleBarcode with the scanned barcode
    res.status(200).json({ message: 'Module barcode updated successfully', moduleBarcode });
}
